...
export function createFlower(params = {}) {
  const flowerColors: FlowerColor[] = [
    "yellow",
    "blue",
    "pink",
    "purple",
  ...
  const opts = Object.assign(
    {
      ...
      petalShape: petalShapes[
        Math.floor(Math.random() * petalShapes.length)
      ] as PetalShape,
  ...
  const stem = new THREE.Mesh(
    new THREE.CylinderGeometry(
      0.02 * opts.petalSize,
      0.03 * opts.petalSize,
      opts.stemHeight * opts.petalSize,
      8,
    ),
  ...
  for (let i = 0; i < opts.petalCount; i++) {
    ...
    if (opts.petalShape === "round") {
      ...
      petalGeo = new THREE.CylinderGeometry(
        petalRadius,
        petalRadius * 0.8,
        petalHeight,
        8,
  ...
  sprite.position.set(
    0,
    opts.stemHeight * opts.petalSize + opts.petalSize * 0.5,
    0,
  );
  group.add(sprite);
