...
function buildBodyGeom(scale = 1) {
  ...
  const geom = new THREE.BoxGeometry(2 * scale, 0.8 * scale, 4 * scale, 3, 2, 5)
  geom.computeVertexNormals()
  return geom
}

function buildTurretGeom(scale = 1) {
  const geom = new THREE.BoxGeometry(1.6 * scale, 0.5 * scale, 1.8 * scale, 2, 2, 3)
...
function buildBarrelGeom(scale = 1) {
  const geom = new THREE.CylinderGeometry(0.08 * scale, 0.12 * scale, 2.2 * scale, 12, 1)
