...
export function createKeep(params = {}) {
  ...
  const keepSprite = createNameSprite('Keep', Math.floor(Math.random() * 10000))
  ...
  const geo = new THREE.BoxGeometry(opts.brickWidth - opts.gap, opts.brickHeight, opts.brickDepth)

  // Simple triplanar-ish shader material (fast approximation)
  const material = createTriplanarMaterial({ baseColor: new THREE.Color(0x8b7d6b) })
  ...
  const instanceMeta: Array<{ hp: number; maxHp: number; defect: number; alive: boolean }> = []
  ...
  for (let layer = 0; layer < opts.layers; layer++) {
    ...
    const staggerOffset = opts.stagger && (layer % 2 === 1) ? (opts.brickWidth / 2) : 0

    for (let slot = 0; slot < opts.bricksPerLayer; slot++) {
      const idx = slotIndex(layer, slot)

      // default metadata
      instanceMeta[idx] = { hp: opts.defaultHP, maxHp: opts.defaultHP, defect: 0, alive: true }
      defectArray[idx] = 0
      ...
      m.compose(new THREE.Vector3(x + staggerOffset, y, z), q, new THREE.Vector3(1, 1, 1))
  ...
  for (let layer = 0; layer < opts.portcullisHeight; layer++) {
    ...
    for (let s = -half; s <= half; s++) {
      const slot = (frontSlotCenter + s + opts.bricksPerLayer) % opts.bricksPerLayer
  ...
  const portWidth = (opts.portcullisWidth / opts.bricksPerLayer) * (Math.PI * 2 * opts.radius) / (2 * Math.PI) // approx arc length -> linear
  const portGeo = new THREE.BoxGeometry(portWidth, opts.portcullisHeight * opts.brickHeight, 0.1)
  const portMat = material.clone()
  const portcullis = new THREE.Mesh(portGeo, portMat)
  portcullis.name = 'portcullis'
  portcullis.position.set(opts.radius + opts.brickDepth * 0.5 + 0.1, (opts.portcullisHeight * opts.brickHeight) / 2, 0)
  group.add(portcullis)

  // Event listeners for destroyed
  const destroyedListeners: Array<(info: any) => void> = []

  function damageBrick(id: number | { layer: number; slot: number }, amount: number) {
    let idx = -1
  ...
  function getBrickData(id: number | { layer: number; slot: number }) {
    ...
    return { index: idx, hp: m.hp, maxHp: m.maxHp, defect: m.defect, alive: m.alive }
  }
...
function createTriplanarMaterial({ baseColor = new THREE.Color(0x8b7d6b) } = {}) {
